...
console.log(ship, showdamage, 'readystatus='+ship.readystatus);
  ...
  vgaPlanets.prototype.setReadyCheckBox = function (object, newState) {
    ...
    if (newState == 1 && vgap.idleActivated) {
      ...
      vgap.refreshLeft();
      //this.needIndexAdjust = true;
    } else if (newState == 0 && vgap.idleActivated) {
      vgap.showRNav(null, object);
      vgap.refreshLeft();
    ...
    if (object.isShip) {
      ...
      if (planet) {
        planet.idleShip = false;
        var ships = vgap.shipsAt(planet.x, planet.y);
        for (var i = 0; i < ships.length; i++) {
          if (ships[i].ownerid == vgap.player.id && ships[i].readystatus == 0) {
            planet.idleShip = true;
            break;
          }
        }
      }
  ...
  vgapStarbaseScreen.prototype.stopBuild = function (nosound) {

    var sb = this.starbase;

    vgap.setReadyCheckBox(sb, 0);

    vgap.logAction("ship build canceled");
    vgap.playSound("button");
    //stop build
    sb.isbuilding = false;

    //sell back unused parts
    this.buyEngine(-1 * vgap.getHull(sb.buildhullid).engines, sb.buildengineid);
    if (sb.buildbeamid > 0)
      this.buyBeam(-1 * sb.buildbeamcount, sb.buildbeamid);
    if (sb.buildtorpedoid > 0)
      this.buyLauncher(-1 * sb.buildtorpcount, sb.buildtorpedoid);
    this.buyHull(-1, sb.buildhullid);

    //lower tech levels
    if (!vgap.settings.isacademy) {
      while (this.changeHullTech(-1)) { }
      while (this.changeEngineTech(-1)) { }
      while (this.changeBeamTech(-1)) { }
      while (this.changeTorpTech(-1)) { }
    }
