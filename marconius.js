...
(function() {
  ...
  sharedContent.prototype.planetScan = function (planet, tempAtTop) {
    ...
    if (planet.nativeclans > 0)
      html += "<hr/><div><span class='nativetype'>" + planet.nativeracename + " "
      + planet.nativegovernmentname + "</span> <span class='raligned "
      + vgap.getHappyClass(planet.nativehappypoints) + "'>" + planet.nativehappypoints
      + ' (' + planet.nativehappychange + ') ' + "</span></div>"
      + "<div class='lval natives'>" + addCommas(planet.nativeclans * 100) + " * "
      + planet.nativetaxrate + "% &rarr; "  + calcIncomeFromNatives() + "</div>";
    ...
    return html;
      ...
      function calcIncomeFromNatives() {
        return Math.floor(planet.nativetaxrate * planet.nativeclans / 100000 * planet.nativetaxvalue * ((planet.nativetype == 6) ? 2 : 1));
      }
  };
